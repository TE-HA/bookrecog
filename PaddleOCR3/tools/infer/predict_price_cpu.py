...
def save_crop_price(img_name, imgs_4, dt_boxes, rec_res):
    ...
    for ii in range(len(imgs_4)):
        ...
        for index in range(num_rec_end):
            if "定价" not in txts[index] and (
                    ("元" or "圆") in txts[index] and '0' in txts[index] and "优" not in txts[index]):
...
def main(_args):
    image_file_list = get_image_file_list(_args.image_dir)
    global text_sys
    text_sys = TextSystem(_args)
    with mp.Pool(mp.cpu_count()) as pool:
        with tqdm(total=len(image_file_list)) as t:
            for i, x in enumerate(pool.imap_unordered(main_tqdm, image_file_list)):
                t.update(1)
